...
plotting_variables=["Mvis","bMass",'nEvent','l1Iso','l2Iso','l1PT','l2PT','l1IP','l2IP','l1IP_SIG','l2IP_SIG','nBPH_Muons','BPH_IP_sig']
TDir=inFile.Get("simple") 

totalevents= TDir.Get("h_nEvents").GetBinContent(1)
channels = ["tauE_tauE","tauMu_tauE","tauMu_tauMu","tauHad_tauE","tauHad_tauMu"]
...
for plot in plotting_variables:
    ...
    for channel in channels:
        ...
        if plot !='nEvent':
            ...
            bphHist = TDir.Get('h_'+channel+"_"+plot+"_BPH")
            bphHist.SetMinimum(0)
            
            stdHist = TDir.Get('h_'+channel+"_"+plot+"_std")
            stdHist.SetMinimum(0)

        else:
            print(plot+"_"+channel)
            bphHist = TDir.Get('h_'+plot+"_"+channel +"_BPH")
            stdHist = TDir.Get('h_'+plot+"_"+channel+"_std")
            bphHist.SetMinimum(.1)
            stdHist.SetMinimum(.1)
            ROOT.gPad.SetLogy()


        bphHist.Scale(2.263/totalevents)
        stdHist.Scale(2.263/totalevents)
        title = bphHist.GetTitle().replace(" (BPH)","")
        bphmax= bphHist.GetMaximum()
        stdmax= stdHist.GetMaximum()
        bphHist.SetLineColor(ROOT.kRed)

        if bphHist.GetMaximum()>stdHist.GetMaximum():
            histMax= bphHist
...
generalPlots = ["h_BPH_Muon_pt","h_BPH_Muon_iso","h_BPH_Muon_IP","h_BPH_Muon_eIP","h_BPH_Muon_IP_sig","h_BPH_Muon_DRB","h_BPH_Muons_nMuons","h_std_electron_selection","h_muon_selection", "h_bJet_selection", "h_nEvents", "h_genWeight"]
